...
def runFairOracles(chosed_atts,current_file,alpha_default=0.05,k_threshold=200,k_percentage=0.5):
    ...
    for si in chosed_atts:
        ...
        for vi in values_si_att:
            ...
            # for missing or unusable simulation data, mark as NA
            if p_value_pairwise is None:
                res_pairwise = "NA"
            else:
...
def computePvaluePairwise(att_name,att_value,current_file, round_default=2):
    ...
    playdata_root = getattr(settings, "PLAYDATA_ROOT", os.path.join(settings.BASE_DIR, "playdata", "rankingfacts"))
    simu_path = os.path.join(playdata_root, "SimulationPairs_N" + str(total_N) + "_R1000.json")
    if not os.path.exists(simu_path):
        # Missing simulation file; signal caller to mark as NA
        return None
    simu_data = read_json_file(simu_path)
    if not simu_data:
        return None
...
def computePvaluePairwise_simu(att_name,att_value,current_file, run_time=100, round_default=2):
    ...
    fair_dir = getattr(settings, "FAIR_GENERATION_DIR", os.path.join(settings.MEDIA_ROOT, "FairRankingGeneration"))
    os.makedirs(fair_dir, exist_ok=True)
    data_file = os.path.join(fair_dir, "FairRankingGeneration")
...
def generateRanking(current_file,top_K=100):
    ...
    filled_data = filled_data.reindex(columns=['GeneratedScore'] + list([a for a in filled_data.columns if a != 'GeneratedScore']))
